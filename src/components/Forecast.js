...
function Forecast() {
  const { forecast, forecastError } = useContext(WeatherContext);
  ...
  return (
    <>
      {!forecastError ? (
        ...
        <div className="text-white flex justify-center items-center sm:text-3xl mt-10 ml-10">
          There was an error fetching the forecast data for this city
        </div>
      )}
    </>
  );
